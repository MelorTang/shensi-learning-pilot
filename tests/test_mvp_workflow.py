...
def test_hermes_parent_can_modify_latest_pending_with_natural_text(tmp_path):
    image_path = tmp_path / "pending-modify-homework.jpg"
    image_path.write_bytes(b"fake pending homework image")
    settings = Settings(
        db_path=tmp_path / "shensi.db",
        vault_path=tmp_path / "vault" / "Shensi-Learning-Vault",
    ...
    ingest = client.post(
        "/ingest/mistake-analysis",
        json={
            "message_id": "hermes-parent-modify-001",
            "platform": "feishu",
            "sender_id": "parent-user",
            "chat_id": "chat-001",
            "image_path": str(image_path),
            "subject": "math",
            "grade": "grade8",
            "note": "parent natural language modify flow",
            "auto_confirm": False,
            "analysis": {
                "provider": "antigravity",
                "model": "gemini-via-antigravity",
                "title": "grade8 slope quiz",
                "concepts": ["linear_function"],
                "error_types": ["calculation_error"],
                "root_cause": "slope formula order mistake",
                "severity": 3,
                "confidence": 0.9,
                "question_items": [
                    {
                    ...
                    {
                        "id": 2,
                        "question": "x+y=10, x-y=2",
                        "student_answer": "x=6,y=4",
                        "is_correct": True,
                    },
                    {
                        "id": 3,
                        "question": "A(1,3), B(5,11), find slope k",
                        "student_answer": "k=-2",
                        "correct_answer": "k=-2",
                        "is_correct": False,
                        "error_reason": "model read the answer as k=-2",
                    },
                ],
            },
        },
    ...
    response = client.post(
        "/hermes/pending/latest/modify",
        json={
            "confirmed_by": "feishu_parent",
            "text": "第3题其实是对的，学生答案改成 k=2，正确答案是 k=2",
        },
    )
    ...
    assert body["question_update_count"] == 1
    assert body["feishu_message"]["msg_type"] == "interactive"
    pending = body["pending"]
    assert pending["found"] is True
    assert pending["questions"][2]["is_correct"] is True
    assert pending["questions"][2]["student_answer"] == "k=2"
    assert pending["questions"][2]["correct_answer"] == "k=2"
    assert pending["confirmation_summary"]["wrong_questions"] == []
    assert "确认卡" in body["reply_text"]
