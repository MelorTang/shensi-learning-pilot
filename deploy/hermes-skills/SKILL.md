...
|---------|--------|-------|
| Bare image received | index + start analysis immediately | `已收到图片，正在分析，约40秒后发确认卡片。` |
...
1. Index the image: `/home/admin/bin/shensi-index-image <chat_id> <sender_id> <path>`
2. **Start analysis immediately:** `/home/admin/bin/shensi-feishu-analysis-latest <chat_id> <sender_id> math grade8 <path>`
...
2. If `found: true`: show the card summary text and tell the parent to use card buttons.
3. If `found: false`: tell the parent: `分析仍在进行中，请稍候。`
4. Do NOT call `shensi-feishu-analysis-latest` again.

## Correction Flow

When the parent sends correction text (e.g., "第3题其实是对的"):
1. Call `shensi-pending-modify <chat_id> <text>`
2. Do NOT call `shensi-feishu-analysis-latest`

## Strictly Forbidden Tools

NEVER call: vision models, `vision_analyze`, `describe_image`, `ocr`,
browser tools, image inspectors.
