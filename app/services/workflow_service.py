...
from app.models.schemas import ConfirmationRequest, LocalUploadRequest, PendingAnalysisModifyRequest
...
class MistakeWorkflowService:
    ...
    def modify_pending_analysis(
        self,
        mistake_id: str,
        request: PendingAnalysisModifyRequest,
    ) -> dict[str, Any]:
        mistake = self.sqlite.get_mistake(mistake_id)
        if not mistake:
            raise ValueError(f"Unknown mistake_id: {mistake_id}")
        if mistake.get("status") != "waiting_confirmation":
            raise ValueError(f"Mistake is not waiting_confirmation: {mistake_id}")
        ...
        now = self._now()
        text_updates = self._parse_parent_modification_text(request.text)
        question_updates = text_updates["question_updates"] + list(request.question_updates or [])
        top_level_overrides = text_updates["overrides"] | dict(request.overrides or {})

        updated = self._apply_overrides(analysis, top_level_overrides)
        changed_fields: list[str] = []
        if top_level_overrides:
            changed_fields.extend(
                key for key in top_level_overrides if key in self._analysis_override_fields()
            )
        question_change_count = self._apply_question_updates(updated, question_updates)
        if question_change_count:
            changed_fields.append("question_items")

        updated["status"] = "waiting_confirmation"
        updated["parent_modifications"] = list(updated.get("parent_modifications") or [])
        updated["parent_modifications"].append(
            {
                "modified_at": now,
                "modified_by": request.confirmed_by,
                "text": request.text,
                "overrides": top_level_overrides,
                "question_updates": question_updates,
            }
        )
        self._refresh_confirmation_summary(updated)

        output_path = self.obsidian.save_ai_output(mistake_id, updated)
        self.sqlite.upsert_mistake(
            mistake
            | {
        ...
        if updated.get("message_id"):
            self.sqlite.update_message_status(updated["message_id"], "waiting_confirmation")

        return {
            "status": "waiting_confirmation",
            "mistake_id": mistake_id,
            "modified": bool(changed_fields),
            "changed_fields": sorted(set(changed_fields)),
            "question_update_count": question_change_count,
            "analysis": updated,
            "confirmation_summary": updated.get("confirmation_summary") or {},
        }
    ...
    def _apply_overrides(self, analysis: dict[str, Any], overrides: dict[str, Any]) -> dict[str, Any]:
        allowed = self._analysis_override_fields()
    ...
    def _analysis_override_fields(self) -> set[str]:
        return {
            "subject",
            "grade",
            "date",
    ...
    def _apply_question_updates(
        ...
        questions = analysis.get("question_items")
        if not isinstance(questions, list) or not updates:
            return 0
        ...
        for raw_update in updates:
            if not isinstance(raw_update, dict):
                continue
            question_id = raw_update.get("id") or raw_update.get("question_id") or raw_update.get("number")
            item = self._find_question_item(questions, question_id)
            if item is None:
                continue
            item_changed = False
            for key, value in raw_update.items():
                if key not in allowed:
                    continue
                if key == "student_steps":
                    value = self._normalize_steps(value)
                if key == "sub_items":
                    value = self._normalize_sub_items(value)
                if item.get(key) != value:
                    item[key] = value
                    item_changed = True
            if item_changed:
                if "needs_parent_review" not in raw_update:
                    item["needs_parent_review"] = False
                verification = item.get("verification")
                if isinstance(verification, dict):
                    verification["previous_status"] = verification.get("status")
                    verification["status"] = "parent_modified"
                    verification["parent_modified"] = True
                changed += 1
        return changed

    def _find_question_item(self, questions: list[Any], question_id: Any) -> dict[str, Any] | None:
        ...
        for index, item in enumerate(questions, start=1):
            if not isinstance(item, dict):
                continue
            if str(item.get("id") or "").strip() == question_id_text:
                return item
            if str(index) == question_id_text:
                return item
        return None

    def _parse_parent_modification_text(self, text: str) -> dict[str, Any]:
        cleaned = " ".join(str(text or "").split())
        if not cleaned:
            return {"overrides": {}, "question_updates": []}

        overrides: dict[str, Any] = {}
        title_match = re.search(r"(?:标题|名称)\s*(?:改成|改为|是)[:：]?\s*(.+)$", cleaned)
        if title_match:
            overrides["title"] = title_match.group(1).strip()

        for field, labels in {
            "root_cause": ("主要原因", "错因", "原因总结"),
            "parent_guidance": ("家长引导", "引导建议", "复习建议"),
            "correct_answer": ("正确答案",),
            "student_answer": ("学生答案", "孩子答案"),
        }.items():
            pattern = rf"(?:{'|'.join(labels)})\s*(?:改成|改为|是)[:：]?\s*(.+)$"
            match = re.search(pattern, cleaned)
            if match and not re.search(r"第\s*\d+\s*题", cleaned):
                overrides[field] = match.group(1).strip()

        updates: list[dict[str, Any]] = []
        matches = list(re.finditer(r"第\s*(\d+)\s*题", cleaned))
        for index, match in enumerate(matches):
            start = match.end()
            end = matches[index + 1].start() if index + 1 < len(matches) else len(cleaned)
            segment = cleaned[start:end].strip(" ，,。；;：:")
            update = self._parse_question_modification_segment(int(match.group(1)), segment)
            if len(update) > 1:
                updates.append(update)
        return {"overrides": overrides, "question_updates": updates}

    def _parse_question_modification_segment(self, question_id: int, segment: str) -> dict[str, Any]:
        update: dict[str, Any] = {"id": question_id}
        if not segment:
            return update

        wrong_terms = ("错", "错误", "不对", "算错", "判错")
        correct_terms = ("对", "正确", "没错", "无误", "做对")
        has_wrong_term = any(term in segment for term in wrong_terms)
        if has_wrong_term:
            update["is_correct"] = False
        if any(term in segment for term in correct_terms) and not any(
            term in segment for term in ("不正确", "不是正确", "不算正确", "不对", "错误", "算错")
        ):
            update["is_correct"] = True

        field_patterns = {
            "correct_answer": r"(?:正确答案|答案)\s*(?:改成|改为|是|应该是)[:：]?\s*([^，,；;。]+)",
            "student_answer": r"(?:学生答案|孩子答案|原答案)\s*(?:改成|改为|是)[:：]?\s*([^，,；;。]+)",
            "error_reason": r"(?:错误原因|错因|原因)\s*(?:改成|改为|是)[:：]?\s*([^，,；;。]+)",
            "concept": r"(?:知识点|考点)\s*(?:改成|改为|是)[:：]?\s*([^，,；;。]+)",
        }
        for field, pattern in field_patterns.items():
            match = re.search(pattern, segment)
            if match:
                update[field] = match.group(1).strip()

        if update.get("is_correct") is True and "error_reason" not in update:
            update["error_reason"] = ""
        return update

    def _refresh_confirmation_summary(self, analysis: dict[str, Any]) -> None:
        question_items = analysis.get("question_items") or []
        if not isinstance(question_items, list):
            question_items = []
            analysis["question_items"] = question_items
        ...
        for item in question_items:
            ...
            if verification.get("status") == "verified":
                ...
                unsupported_count += 1
            if verification.get("conflict_with_llm"):
                conflict_count += 1
        math_verification = dict(analysis.get("math_verification") or {})
        math_verification |= {
            "total_count": len(question_items),
            "verified_count": verified_count,
            "unsupported_count": unsupported_count,
            "conflict_count": conflict_count,
            "needs_parent_review_count": len(
                [
                    item
                    for item in question_items
                    if isinstance(item, dict) and item.get("needs_parent_review")
                ]
            ),
        }
        analysis["math_verification"] = math_verification
        analysis["extracted_question_count"] = len(question_items)
        analysis["confirmation_summary"] = self._build_confirmation_summary(
            question_items,
            math_verification,
